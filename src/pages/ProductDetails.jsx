...
export default function ProductDetails() {
  ...
  const products = [
    ...
    { id: 3, name: "Latte", desc: "Caffè Misto is a simple and comforting coffee made with equal parts brewed coffee and steamed milk, creating a smooth, balanced flavor that is less intense than a latte.", img: latte, price: 140 },
  ...
  return (
    <section className="min-h-screen bg-[#C7AD7F]">
      <div className="relative w-full max-w-2xl mx-auto h-[40vh] sm:h-[65vh] ">
        ...
        >
          <Heart
            size={50}
            className={`transition-colors ${isFavorited ? "fill-black text-black" : "text-black"}`}
          />
        </button>
      </div>

      <div className="bg-white max-w-2xl mx-auto -mt-5 sm:-mt-16 rounded-t-3xl p-6 sm:p-10 pb-18 sm:pb-12 relative z-10">
        <div className="flex items-center justify-between">
          <h1 className="text-4xl sm:text-3xl lg:text-4xl font-semibold text-black">{product.name}</h1>
          <span className="text-3xl sm:text-xl lg:text-2xl text-black">₱ {computedPrice}</span>
        </div>

        <p className="text-black text-sm sm:text-base lg:text-lg mt-3 leading-relaxed border-b pb-4">{product.desc}</p>

        <h2 className="mt-3 mb-3 text-2xl sm:text-xl text-black">Flavor</h2>
        <div className="flex gap-4 flex-wrap border-b pb-5 pl-3">
          {flavors.map((f) => (
            <button
              key={f}
        ...
        <div className="flex items-center justify-between mt-5">
          <h2 className="mt-3 mb-3 text-3xl sm:text-xl text-black">Size</h2>
          <div className="flex items-center gap-4">
            {sizes.map((s) => (
              <button
                key={s}
        ...
        <button
          ...
          className="w-full bg-[#C0A16B] text-black py-2 mt-10 text-3xl sm:text-xl font-semibold border-2 hover:opacity-90 transition"
        >
          Add to Cart
        </button>
      </div>
